...
export class TruckService {
        ...
        if (user.role !== 'admin') {
          try {
            ...
            if (truckTypeLabel !== 'TANKER') {
              description += ` Capacity: ${newTruck.capacity || ''}, Depot: ${newTruck.depot || ''}.`;
  ...
  const { page = 1, limit = 10, search, profileId, status, depotHubId, productId, size, loadStatus, truckType, locationId } = query;
    let offset = 0;
    if (page && page > 0) {
      offset = (page - 1) * limit;
    }
